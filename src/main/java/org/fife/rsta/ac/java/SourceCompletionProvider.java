...
public class SourceCompletionProvider extends DefaultCompletionProvider {
	...
	/**
	 * Adds completions for accessible methods and fields of super classes. This
	 * is only called when the caret is inside of a class. TODO: Handle
	 ...
	 * @param cf
	 *            A class in the chain of classes that a type being parsed
	 *            inherits from.
	 * @param pkg
	 *            The package of the source being parsed.
	 * @param typeParamMap
	 *            A mapping of type parameters to type arguments for the object
	 ...
	 * @param method
	 * @param offs
	 *            The caret's offset into the source. This should be inside of
	 *            <code>method</code>.
	 */
	 ...
	 *
	 * @param info
	 *            The jar to add. If this is <code>null</code>, then the current
	 ...
	 *
	 * @param type
	 *            The type of a variable/field/etc. whose fields/methods/etc.
	 *            are being code completed, as declared in the source. This
	 *            includes type arguments.
	 * @param cf
	 *            The <code>ClassFile</code> representing the actual type of the
	...
	protected List<Completion> getCompletionsImpl(JTextComponent comp) {
		...
		try {
			...
			if (!stringLiteralMember) {
				...
				// log.info("jar completions loaded in: " + time);

				// Loop through all types declared in this source, and provide
				// completions depending on in what type/method/etc. the caret's
				// in.
...
//		int dot = comp.getCaretPosition();
	 ...
	 * fields in the class are loaded. Methods and fields in super classes are
	 * also loaded. TODO: Get super methods/fields added correctly by access!
	 * <li>If the caret is in a field, local variables currently accessible are
	 * loaded.
	 * </ul>
	 *
	 * @param cu
	...
	private void loadCompletionsForCaretPosition(CompilationUnit cu, JTextComponent comp, String alreadyEntered,
		...
		// long time = System.currentTimeMillis() - startTime;
		// log.info("methods/fields/localvars loaded in: " + time);

	}

	public static Method findCurrencMethod(CompilationUnit cu, JTextComponent comp,
	...
	/**
	 * Loads completions based on the current caret location in the source. This
	 * method is called when the caret is found to be in a specific type
	 ...
	 */
	private void loadCompletionsForCaretPosition(CompilationUnit cu, JTextComponent comp, String alreadyEntered,
			Set<Completion> retVal, TypeDeclaration td, String prefix, int caret) {
		...
		Method currentMethod = findCurrencMethod(cu, comp, td,caret);
			...
			if (td instanceof NormalClassDeclaration) {
				...
				if (extended != null) { // e.g., not java.lang.Object
					...
					if (cf != null) {
						...
						log.info("[DEBUG]: Couldn't find ClassFile for: " + superClassName);
					}
				}
			}
		}
	...
	private MethodInfo findMethod(ClassFile cf, String methodName, int paramCount) {
		...
		if (methodInfoByName == null) {
			//log.info("can't find methods in  ");
		...
		if (methodInfoByName.size() > 1) {
			log.info(methodInfoByName.size() + " " + methodName);
		}
		...
		if (fieldInfo == null) {

			{
				...
				if (s != null && s.length() > 1) {
					log.info("can't find methods for ");
	...
	private Type resolveTypeWithDot(CompilationUnit cu, String alreadyEntered,
		...
		if (afterDot.contains("(")) {
			...
			log.info(methodName + " " + countMethodParams);
			{
			...
			if (fieldInfoByName == null) {
				log.info("can't fin fields '" + afterDot + "' , " + prefix + " " + cf.getClassName(true));
				return null;
			}
	...
	private void resolveType2WithDot(CompilationUnit cu, String alreadyEntered, TypeDeclaration td,
			Method currentMethod, String prefix, int offs, int dot) {
		String beforeDot = prefix.substring(0, dot).trim();
		String afterDot = prefix.substring(dot + 1).trim();
		log.info(beforeDot);
		log.info(afterDot);
		...
		if (afterDot.contains("(")) {
			...
			{
				MethodInfo methodInfo = findMethod(cf, methodName, countMethodParams);
				OpenMember.openMember.openMember(methodInfo);
				return ;
			}
		} else if (afterDot.contains("[")) {
			log.info("arrays not supported");
			...
			FieldInfo fieldInfoByName = findField(cf, afterDot);
			if (fieldInfoByName == null) {
				log.info("can't find fields '" + afterDot + "' , " + prefix + " " + cf.getClassName(true));
				return ;
			}
			OpenMember.openMember.openMember(fieldInfoByName);;
		}
		log.info("not found : " +prefix);
	}
	...
	private Type resolveType2(CompilationUnit cu, String alreadyEntered, TypeDeclaration td,
		...
		}
		{
			...
			if(matchedImports.size()==1) {
				log.info("found matched imports ");
				return new Type(matchedImports.get(0).getClassName(true));
			}
			if(matchedImports.size()>0) {
				log.info("found matched imports ,ore than 1");
			}else {
				// log.info("found matched imports not found");
			}
		}
		log.info("not found " + prefix);
		return null;
	...
	void open(CompilationUnit cu, String alreadyEntered, TypeDeclaration td,
		...
		int dot = prefix.lastIndexOf(".");
		if (dot > -1) {
			resolveType2WithDot(cu, alreadyEntered,  td, currentMethod, prefix, offs, dot);
			return;
		}
		{
			List<ClassFile> matchedImports=new ArrayList();
			...
			if(matchedImports.size()==1) {
				log.info("found matched imports ");
				OpenMember.openMember.openClass(matchedImports.get(0));
			}
			if(matchedImports.size()>0) {
				log.info("found matched imports ,ore than 1");
			}else {
	 ...
	 *            to be non-<code>null</code> not equal to "<tt>this</tt>".
	 * @param offs
	...
	private void loadCompletionsForCaretPositionQualified(CompilationUnit cu, String alreadyEntered,
			Set<Completion> retVal, TypeDeclaration td, Method currentMethod, String prefix, int offs) {
	...
	private void loadCompletionsForCaretPositionQualifiedCodeBlock(CompilationUnit cu, Set<Completion> retVal,
	...
	private void loadImportCompletions(Set<Completion> set, String text, CompilationUnit cu) {
		...
		// long time = System.currentTimeMillis() - startTime;
		// log.info("imports loaded in: " + time);
